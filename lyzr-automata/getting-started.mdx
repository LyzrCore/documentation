...
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| parameters | All existing parameters supported by OpenAI's chat completion model are included by default for customizable requests. Users can pass these using the parameters dictionary. |
| api_key    | The api_key parameter is the user's OpenAI API key, required for authentication to access the chat completion service.                                                       |
...
| Parameter  | Description                                                                                                                                                                   |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| parameters | All existing parameters supported by OpenAI's image generation model are included by default for customizable requests. Users can pass these using the parameters dictionary. |
| api_key    | The api_key parameter is the user's OpenAI API key, required for authentication to access the image generation service.                                                       |
...
| Parameter  | Description                                                                                                                 |
| ---------- | --------------------------------------------------------------------------------------------------------------------------- |
| parameters | A dictionary supporting all existing parameters accepted by the Perplexity chat completion model for customizable requests. |
| api_key    | The user's Perplexity API key required for authentication to access the chat completion service.                            |
...
You can bring in your own model and connect it to our framework by extending our base AIModel class.
    ...
    1. Import AIModel class
    2. You may provide initial parameters, including the API key and any model-specific parameters.
    3. Crucially, create two methods: **generate_text** and **generate_image**, as stipulated by the abstract class design of AIModel.
...
| Parameter      | Description                                                                                                                                                                                                   |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| task_id        | An optional identifier for the task. Used to specify or reference a particular task instance.                                                                                                                 |
| system_persona | An optional string that defines the persona or role of the system in the conversation.                                                                                                                        |
| prompt         | An optional string containing the initial prompt or question posed by the user to the AI system.                                                                                                              |
| messages       | An optional list of dictionaries, each representing a message in the conversation history. The default value creates a conversation starting with the system persona's message followed by the user's prompt. |
...
| Parameter    | Description                                                                                                                                                                     |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| task_id      | A required identifier for the task, used to specify a unique instance for image generation and storage.                                                                         |
| prompt       | The prompt based on which the image will be generated. This string should describe the desired outcome for the image.                                                           |
| resource_box | An instance of ResourceBox, which is used for managing the storage of the generated image, including saving the file from a URL and determining the subfolder based on task_id. |
...
| Parameter      | Description                                                                                                 |
| -------------- | ----------------------------------------------------------------------------------------------------------- |
| role           | Defines the agent's operational role or function, setting the context for its tasks and interactions.       |
| prompt_persona | Describes the intended persona or style the agent should adopt, influencing its approach to task execution. |
| memory         | Specifies the memory system instance the agent uses to interact with vector store memory.                   |
...
| Parameter       | Description                                                                                                                                                                                       |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| name            | The name of the tool, giving a clear indication of its functionality or the task it is designed to perform.                                                                                       |
| desc            | A brief description of the tool's purpose and how it operates, providing context to its users.                                                                                                    |
| function        | The specific function the tool executes. This should be a callable object that performs the tool's main operation.                                                                                |
| function_input  | The Pydantic model that defines the structure, type, and validation rules for the input data. This ensures that the function receives data in the expected format.                                |
| function_output | The Pydantic model for the output data, specifying what the function returns. This model validates the output data structure and type, ensuring consistency and reliability in the tool's output. |
...
| Parameter      | Description                                                                                                                                                                                                    |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| name           | The unique identifier or descriptive name for the task, highlighting its primary focus or objective.                                                                                                           |
| agent          | The agent instance responsible for carrying out the task, equipped with the necessary skills or functionalities. Instance of Agent class                                                                       |
| output_type    | Specifies the format or type of the task's output, ensuring compatibility with subsequent operations or requirements. accepts ( OutputType.TEXT / OutputType.IMAGE )                                           |
| input_type     | Defines the expected format or type of input the task requires, aligning with the nature of the data being processed. ( InputType.TEXT / InputType.IMAGE )                                                     |
| model          | (Optional) A reference to a the llm model. Instance of AIModel ( OpenAIModel/ PerplexityModel ) class.                                                                                                         |
| instructions   | Detailed guidance or commands that the agent follows to execute the task, potentially including objectives and constraints.                                                                                    |
| log_output     | A boolean value indicating whether the task's outputs should be streamed to logger.                                                                                                                            |
| enhance_prompt | A boolean flag enabling additional processing or modification of the initial instructions to optimize task performance. ( Automatically enhances prompts based on https://arxiv.org/pdf/2312.16171.pdf paper ) |
| default_input  | A predefined input value used when actual input is unavailable or incomplete, ensuring task continuity.                                                                                                        |
| tool           | Tool is a instance of Tool class to do external software actions using llm flow Eg. email sending tool                                                                                                         |
| input_tasks    | Input tasks instances of Task class, it is for explicitly mentioning which tasks output to be used as input for current task.                                                                                  |
